        ...
        db.serialize(function() {  
            ...
            db.all(sql1, [filmId], function(err, check) {
                if (err) {
                    ...
                    db.all(sql2, [userId, filmId], function(err, rows) {
                        if (err) {
                            db.run('ROLLBACK;')
                            reject(err);
                        } else {
                            const sql3 = 'UPDATE reviews SET active = 0 WHERE reviewerId = ?';
                            db.run(sql3, [userId], function(err) {
                                if (err) {
                                    db.run('ROLLBACK;')
                                    reject(err);
                                } else {
                                    const sql4 = 'UPDATE reviews SET active = 1 WHERE reviewerId = ? AND filmId = ?';
                                    db.run(sql4, [userId, filmId], function(err) {
                                        if (err) {
                                            db.run('ROLLBACK;')
                                            reject(err);
                                            ...
                                            reject(403);
                                                    ...
                                                    var updateMessage = new WSMessage('update', parseInt(userId), rows[0].name, parseInt(filmId), rows[0].title);
                                                    WebSocket.sendAllClients(updateMessage);
                                                    WebSocket.saveMessage(userId, new WSMessage('login', parseInt(userId), rows[0].name, parseInt(filmId), rows[0].title));
